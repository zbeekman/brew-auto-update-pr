...
if [ "Xtrue" = "X$(jq -r '.release.prerelease' "$GITHUB_EVENT_PATH")" ]; then
...
export BREW_PR_MSG="${BREW_PR_MSG:-Created with brew-auto-update-pr.}"
...
if [ "Xhttps://github.com/${GITHUB_REPOSITORY}/archive" = "${old_url%/*}" ]; then
    echo "Formula ${BREW_PR_FORMULA} appears to use default archves created by github"
...
if [ ! "$num_release_assets" -gt 0 ]; then
    ...
    if [ ! "X$new_release_present" = "Xtrue" ]; then
	...
	case "$f" in
	    *sha256*|*SHA256*)
		if grep "${new_file}" "$f" ; then
		    ...
		    grep "${new_file}" "$f"
		    sha256sum "${new_file}"
		    if [ "X$checksum_fault" = "Xtrue" ]; then
			exit 99
		    fi
		fi
    ...
    if [ ! "X$checksum_verified" = "Xtrue" ]; then
	echo "WARNING: Computing sha256 checksum on the fly from ${new_url!}" >&2
